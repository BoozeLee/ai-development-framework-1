...
class AIOSBuilderAgent:
    def __init__(self, name="AIOSBuilder"):
        ...
        self.name = name
        self.status = State(
            ["idle", "analyzing", "building", "testing", "done", "error"],
            name="status",
            default="idle",
        )
        ...
        try:
            ...
            for dep in aios_dependencies:
                try:
                    ...
                    result = subprocess.run(
                        [pip_cmd, "install", dep], capture_output=True, text=True
                    )
                    if result.returncode == 0:
...
"""
            ...
            result = subprocess.run(
                [str(self.aios_env / "bin" / "python"), "-c", aios_test],
                capture_output=True,
                text=True,
            )
...
"""
            ...
            result = subprocess.run(
                [str(self.aios_env / "bin" / "python"), "-c", integration_test],
                capture_output=True,
                text=True,
            )
            ...
            if result.returncode == 0:
                ...
                self.logger.error(
                    f"❌ AI stack integration test failed: {result.stderr}"
                )
                return False
...
"""
            ...
            # Save workflow
            workflow_file = (
                Path.home() / ".aios" / "templates" / "aios_productivity_workflow.py"
            )
...
if __name__ == "__main__":
    ...
    if success:
        ...
        print("\\nNext steps:")
        print(
            "1. Test the system: python ~/.aios/templates/aios_productivity_workflow.py"
        )
