...
"""
    ...
    def create_agent(self):
        ...
        agent_type = input("Enter agent type (researcher/coder/analyst/assistant): ").strip()
        properties = {}
...
'''
    ...
    def run_builder_agent(self):
        ...
        if self.activate_environment():
            try:
                subprocess.run([f"{self.env_path}/bin/python", str(builder_script)], check=True)
            except subprocess.CalledProcessError as e:
    ...
    def show_agent_status(self):
        ...
        # Show system status
        print(f"\n📈 System Status:")
        print(f"  • AIOS Environment: {'✅ Active' if self.env_path.exists() else '❌ Not found'}")
    ...
    def debug_system(self):
        ...
        if self.activate_environment():
            try:
                subprocess.run([f"{self.env_path}/bin/python", str(debug_script)], check=True)
            except subprocess.CalledProcessError as e:
...
Enter choice (1-6): """)
    ...
    def install_package(self):
        ...
        if self.activate_environment():
            try:
                subprocess.run([f"{self.env_path}/bin/pip", "install", package], check=True)
    ...
    def uninstall_package(self):
        ...
        if self.activate_environment():
            try:
                subprocess.run([f"{self.env_path}/bin/pip", "uninstall", "-y", package], check=True)
    ...
    def update_packages(self):
        ...
        if self.activate_environment():
            try:
                subprocess.run([f"{self.env_path}/bin/pip", "install", "--upgrade", "pip"], check=True)
    ...
    def setup_ai_stack(self):
        ...
        if self.activate_environment():
            try:
                subprocess.run([f"{self.env_path}/bin/python", str(stack_script)], check=True)
            except subprocess.CalledProcessError as e:
